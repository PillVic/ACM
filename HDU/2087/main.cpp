...
vector<int> buildNext(const string& pattern) {
    ...
    // next数组:使得 next[i]=k 表示
    // p[0]开头长度为前缀串与p[i]结尾的后缀串相同的最大长度为k
...
//kmp算法升级版,直接替换Next数组就能加速,不需要对其他代码作任何改动
vector<int> buildNextVal(const string& pattern, const vector<int> next) {
    vector<int> nextval(next.size());
...
int kmpMatch(const string& text, const string& pattern) {
    ...
    vector<int> nextval = buildNextVal(pattern, next);
    ...
    while (textPos < text.size()) {
        if (text[textPos] == pattern[patternPos]) {
             ...
             */
            patternPos = nextval[patternPos - 1];
        } else {
            // 模式串开头失败，直接下一个字符
            textPos += 1;
        }
